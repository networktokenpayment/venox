...
#if defined(NDEBUG)
#error "Venox cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 15 * COIN;
    } else if (nHeight < 755600) {
        nSubsidy = 10 * COIN;
    } else if (nHeight < 1043999) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 1562398) {
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Venox
...
{
    RenameThread("venox-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZVNXMinted()
{
...
void RecalculateZVNXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zVNX supply
...
bool RecalculateVNXSupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert VNX to zVNX
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Venox: We use certain sporks during IBD, so check to see if they are
