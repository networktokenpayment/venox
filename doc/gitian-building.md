...
*Setup instructions for a gitian build of Venox using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Venox
...
Clone the git repositories for venox and gitian and then checkout the venox version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the venox repository
...
To build Venox (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the venox repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/venox/.git/
...
./bin/gbuild --commit venox=${COMMIT} --url venox=${URL} ../Venox/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/venox-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/venox-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/venox-build.assert
```
